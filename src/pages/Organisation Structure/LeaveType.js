...
const deleteHandler=(e,id)=>{
  e.preventDefault()
  axios.delete(`http://localhost:8080/leavetype/${id}`).then(res=>console.log(res))
}
useEffect(()=>{
  
  fetch("http://localhost:8080/LeaveType").then(res=>res.json()).then(value=>setContacts(value))
},[contacts])
const showHandler=(e)=>{
  e.preventDefault();
  setShow(!show);
...
const submitHandler=(event)=>{
  ...
  return (
    ...
    <h4>Leave Type</h4>   <button onClick={showHandler} type="submit" class="btn btn-primary sm-4 mt-2">Add Leave Type</button>
